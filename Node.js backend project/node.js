...
app.post('/api/moniepoint-create-account', express.json(), async (req, res) => {
  const { name, phone } = req.body;
  const apiSecret = process.env.MONIEPOINT_API_SECRET;
  try {
    // Replace with Moniepoint's real endpoint and payload
...
app.post('/api/moniepoint-webhook', express.json(), (req, res) => {
  const event = req.body;
  // Example: event contains { account_name, amount, status }
